    ...
    *skip_shapes* is *True*, all shape files remain unchanged (the shape lines in the datacard
    ...
    with manipulate_datacard(datacard) as content:
        ...
        if len(bin_names) != len(param_values):
            raise Exception("number of bins and processes ({}) not matching values of parameter {} "
                "({})".format(len(bin_names), param_name, len(param_values)))
        ...
        for new_name, patterns in specs.items():
            ...
            for bin_name, process_name, value in zip(bin_names, process_names, param_values):
                ...
                for bin_pattern, process_pattern in patterns:
                    ...
                    # store the value
                    spec_value = value
                    logger.debug("spec of new parameter {} matched bin {} and process {}, "
                        "assigning value {}".format(new_name, bin_name, process_name, value))
                    break
