...
def create_shape_plot(bin_name, proc_label, proc_shapes, param, directory, file_type, nom_format,
    ...
    if plot_syst:
        ...
        if y_min2 is None:
	    y_min2 = min(min(h.GetBinContent(b) - h.GetBinErrorLow(b) for b in range(1, h.GetNbinsX() + 1)) for h in [hist_d_trans2, hist_u_trans2, hist_n_trans2])
            y_min2 = min(-0.059, max(-59., y_min2 * 1.2))
        if y_max2 is None:
	    y_max2 = max(max(h.GetBinContent(b) + h.GetBinErrorUp(b) for b in range(1, h.GetNbinsX() + 1)) for h in [hist_d_trans2, hist_u_trans2, hist_n_trans2])
        ...
        # add to plots
        draw_objs2.append((hist_d_trans2, "SAME,HIST,E1"))
        draw_objs2.append((hist_u_trans2, "SAME,HIST,E1"))
        draw_objs2.append((hist_n_trans2, "SAME,HIST,E1"))
