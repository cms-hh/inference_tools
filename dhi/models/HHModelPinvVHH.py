...
class HHModel(PhysicsModel):
    ...
    def __init__(self, ggf_sample_list, vbf_sample_list, vhh_sample_list, name):
        ...
        self.vhh_formula = VHHFormula(vhh_sample_list)
    ...
    def check_validity_vhh(self, vhh_sample_list):
        if len(vhh_sample_list) < 6:
            raise RuntimeError("%s : malformed VHH input sample list - expect at least 6 samples" % self.name)
        if not isinstance(vhh_sample_list, list) and not isinstance(vhh_sample_list, tuple):
            raise RuntimeError("%s : malformed VHH input sample list - expect list or tuple" % self.name)
        for s in vhh_sample_list:
            if not isinstance(s, VHHSample):
                raise RuntimeError("%s : malformed VHH input sample list - each element must be a VHHSample" % self.name)
    ...
    def reset_pois(self):
        self.r_pois = ["r", "r_gghh", "r_qqhh", "r_vhh"]
        self.k_pois = [
    ...
    def doParametersOfInterest(self):
        ...
        ## VHH = r_VHH x [sum samples(kl, CV, C2V)]
        ...
        self.modelBuilder.doVar("r_vhh[1,-20,20]")
    ...
    def create_scalings(self):
        ...
        for i, s in enumerate(self.vhh_formula.sample_list):
            # f_name = 'f_vhhscale_sample_{0}'.format(i)
            f_name = 'f_vhhscale_sample_{0}'.format(s.label)
            ...
            exprname = 'expr::{}(\"{}\" , {})'.format(f_name, s_expr, arglist)
            # print exprname
            self.modelBuilder.factory_(exprname) # the function that scales each VHH sample

            f_prod_name_pmode = f_name + '_r_vhh'
            prodname_pmode = 'prod::{}(r_vhh,{})'.format(f_prod_name_pmode, f_name)
            self.modelBuilder.factory_(prodname_pmode)  ## the function that scales this production mode
            # self.modelBuilder.out.function(f_prod_name_pmode).Print("") ## will just print out the values
    ...
    def done(self):
        ...
        # get the labels of ggF, VBF and VHH samples and store a flag to check if they were matched
        ...
        for prefix, matches in [("ggHH", matches_ggf), ("qqHH", matches_vbf), ("VHH", matches_vhh)]:
        ...
        # print matches
        matches = list(matches_ggf.items()) + list(matches_vbf.items()) + list(matches_vhh.items())
...
def get_vhh_samples(keys):
    """
    Returns a list of :py:class:`VHHSample` instances that are mapped to certain *keys* in the
    ordered *vhh_samples* dictionary above. A key can either be a tuple of (CV, C2V, kl) values as
    used in the dict, or a numeric index. Example:
...
def create_model(name, skip_ggf=None, skip_vbf=None, skip_vhh=None):
    ...
    using all availabe samples except those defined in *skip_ggf*, *skip_vbf* and *skip_vhh*,
    respectively, through :py:func:`get_ggf_samples`, :py:func:`get_vbf_samples` and
    :py:func:`get_vhh_samples`. The order of passed keys to be skipped does not matter.
    """
...
def create_hh_xsec_func(ggf_formula=None, vbf_formula=None, vhh_formula=False):
    """
...
get_vhh_xsec = create_vhh_xsec_func(model_default.vhh_formula)

#: Default combined cross section getter using the formulas of the *model_default* model.
get_hh_xsec = create_hh_xsec_func(model_default.ggf_formula, model_default.vbf_formula, False)
