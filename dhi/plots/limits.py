...
def plot_limit_scan(
    ...
    *show_excluded_ranges* is *True* the excluded ranges are marked with a hatched area. The
    ...
    r.setup_graph(
        g_exp,
        props={"LineWidth": 2, "LineStyle": 2, "MarkerStyle": 20, "MarkerSize": 1},
    )
    ...
    if has_thy or not xsec_unit:
        ...
        allowed_ranges[key_exp] = excluded_to_allowed_ranges(
            excl_ranges_exp,
            x_min_scan,
            x_max_scan,
        )
    ...
    if has_obs:
        ...
        if has_thy or not xsec_unit:
            ...
            allowed_ranges[key_obs] = excluded_to_allowed_ranges(
                excl_ranges_obs,
                x_min_scan,
                x_max_scan,
            )
...
def plot_limit_points(
    ...
    *cms_postfix* is shown as the postfix behind the CMS label.
    ...
    x_title = "95% CL limit on {} {}".format(
        to_root_latex(create_hh_xsbr_label(poi, hh_process, process_label)),
        to_root_latex("({})".format(xsec_unit)) if xsec_unit else "/ #sigma_{Theory}",
    )
...
def plot_limit_scan_2d(
    ...
    *h_lines* and *v_lines* can be sequences of float values denoting positions of horizontal and
    ...
    if "unit" in poi_data[scan_parameter1]:
        x_title = "{} ({})".format(x_title, to_root_latex(poi_data[scan_parameter1].unit))
