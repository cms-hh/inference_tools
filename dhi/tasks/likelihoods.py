...
class MergeLikelihoodScan(LikelihoodBase):
    ...
    def run(self):
        ...
        for branch, inp in self.input()["collection"].targets.items():
            ...
            if not poi_mins:
                mins = tuple(f[p].array()[0] for p in self.scan_parameter_names)
                if np.isfinite(mins).all():
                    poi_mins = np.array(
                        mins, dtype=[(p, float) for p in self.scan_parameter_names],
                    )
...
class PlotLikelihoodScan(LikelihoodBase, POIPlotTask):
    ...
    def _load_scan_data(
        ...
        for inp in inputs:
            ...
            print(scan_parameter_names, data["poi_mins"])
            all_poi_mins.append([
