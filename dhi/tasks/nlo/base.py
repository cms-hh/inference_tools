...
class POITask(DatacardBaseTask):
    ...
    def get_freeze_parameters(self):
        return ",".join(self.other_pois)

    def get_set_parameters(self):
        return ",".join([p + "=1" for p in self.other_pois])


class POITask1D(POITask):

    poi = luigi.ChoiceParameter(default="kl", choices=POITask.k_pois, description="name of the "
        "poi; default: kl")
    poi_value = luigi.FloatParameter(default=1.0, description="initial value of the poi; default: "
        "1.0")
...
class POIScanTask1D(POITask1D):
    ...
    poi_range = law.CSVParameter(cls=luigi.IntParameter, default=(-10, 10), min_len=2, max_len=2,
...
class POITask2D(POITask):
    ...
    poi1 = luigi.ChoiceParameter(default="kl", choices=POITask.k_pois, description="name of "
        "the first poi; default: kl")
    poi2 = luigi.ChoiceParameter(default="kt", choices=POITask.k_pois, description="name of "
        "the first poi; default: kt")
    poi1_value = luigi.FloatParameter(default=1.0, description="initial value of the first poi; "
        "default: 1.0")
    poi2_value = luigi.FloatParameter(default=1.0, description="initial value of the second poi; "
        "default: 1.0")
...
class POIScanTask2D(POITask2D):
    ...
    poi1_range = law.CSVParameter(cls=luigi.FloatParameter, default=(-10., 10.), min_len=2,
        max_len=2, description="the range of the first poi given by two comma-separated values; "
        "default: -10,10")
    poi2_range = law.CSVParameter(cls=luigi.FloatParameter, default=(-10., 10.), min_len=2,
        max_len=2, description="the range of the second poi given by two comma-separated values; "
        "default: -10,10")
    points1 = luigi.IntParameter(default=21, description="number of points of the first poi to "
        "scan; default: 21")
    points2 = luigi.IntParameter(default=21, description="number of points of the second poi to "
        "scan; default: 21")
