...
action() {
    ...
    [ -z "$DHI_JOB_DIR" ] && export DHI_JOB_DIR=""$this_dir"/data/jobs"
    ...
    if [ ! -f "$DHI_SOFTWARE/.good" ]; then
        ...
        LAW_INSTALL_EXECUTABLE=env dhi_pip_install git+https://github.com/riga/law.git@fix/remote_task_parameters --no-binary law || return "$?"
