...
action() {
    ...
    local docs_dir="$( dirname "$this_dir" )"
    local root_dir="$( dirname "$docs_dir" )"
    ...
    local docker_cmd=""
    if [ "$mode" = "build" ]; then
        docker_cmd="bash -c \"cd docs && mkdocs build --strict\""
        docker_args="$docker_args -t"
    elif [ "$mode" = "serve" ]; then
        local host_port="${2:-8000}"
        docker_cmd="bash -c \"cd docs && mkdocs serve --dev-addr 0.0.0.0:8000\""
    ...
    # start the container
    local cmd="docker run $docker_args gitlab-registry.cern.ch/hh/tools/inference $docker_cmd"
    echo -e "command: $cmd\n"
    eval "$cmd"
}
action "$@"
